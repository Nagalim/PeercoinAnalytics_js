...
document.getElementById('priceInput').onchange = function () {
        try{
                let pfiletext = this.value.replace("C:\\fakepath\\", "");
                alert(pfiletext);
                document.getElementById('pricefilearea').innerHTML=pfiletext;
                document.getElementById('pricefilebtn').style.background='#008000';
                document.getElementById('imppricebtn').style.background='#FF0000';
        }catch(err){alert(err)};
}
...
function impprice()
{
        ...
        try{
                ...
                preader.onload = function (e) {rawpricedata = e.target.result};
                preader.onloadend = function () {procprice(rawpricedata)};
                preader.readAsText(document.getElementById('priceInput').files[0]);
                document.getElementById('imppricebtn').style.background='#008000';
        }
        catch(err){alert(err);}
}

function procprice(rawpricedata)
{
        var prows = rawpricedata.split("\n");
        var pdated = [], pamnt = [];
        for (let p=1;p<prows.length;p++){
                let pthisrow = prows[p].split(',');
                let pdating = new Date(pthisrow[0].replaceAll('-','/'));
                pdated.push(pdating);
                pamnt.push(pthisrow[1]);
        }
        priceexp(pdated,pamnt)
}

function priceexp(pdated,pamnt)
{
        var stakedate = []; stakeprice = [];
        let i=-1;
        var mind = Date.parse(document.getElementById('windowstart').value);
        var maxd = Date.parse(document.getElementById('windowend').value);
        var pcsv = "Date,Amount (USD)\n";
        while(resdata[++i]){
                if (resdata[i][0]>mind && resdata[i][0]<maxd) {
                        if (resdata[i][2] == "Mint by stake")
                        {
                                stakedate.push(resdata[i][4]);
                                let pindex=pdated.findIndex(function(ptime) {
                                        return ptime > resdata[i][4];
                                });
                                stakeprice.push(parseFloat(resdata[i][1])*pamnt[pindex]);
                                let stakeinfo = [];
                                stakeinfo.push(resdata[i][4]);
                                stakeinfo.push(parseFloat(resdata[i][1])*pamnt[pindex]);
                                pcsv += stakeinfo.join(',');
                                pcsv += "\n";
                        }
                }
        ...
        hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodeURI(pcsv);
        hiddenElement.target = '_blank';
        hiddenElement.download = 'mintevents.csv';
        hiddenElement.click();
}
